...
class Image {
    constructor(width, height, data, options) {
        ...
        var kindDefinition = (0, _extend2.default)({}, theKind, options);
    ...
    static createFrom(other, options) {
        ...
        (0, _extend2.default)(newOptions, options);
    ...
    toDataURL(type = 'image/png', options = {}) {
        ...
        var _options = options,
            ...
            _options$useCanvas = _options.useCanvas,
            useCanvas = _options$useCanvas === undefined ? false : _options$useCanvas,
            _options$encoder = _options.encoder,
            encoderOptions = _options$encoder === undefined ? undefined : _options$encoder;

        type = (0, _mediaTypes.getType)(type);
        function dataUrl(encoder, ctx) {
            var u8 = encoder(ctx, encoderOptions);
            return (0, _base.toBase64URL)(u8, type);
        }
        if (async) {
            return new Promise((resolve, reject) => {
                if (type === 'image/bmp') {
                    resolve(dataUrl(_fastBmp.encode, this));
                } else if (type === 'image/png' && canJSEncodePng(this) && !useCanvas) {
                    resolve(dataUrl(_fastPng.encode, this));
                } else {
            ...
            if (type === 'image/bmp') {
                ...
                return dataUrl(_fastPng.encode, this);
            } else {
    ...
    save(path, options = {}) {
        var _options$format = options.format,
            ...
            _options$encoder2 = options.encoder,
            encoderOptions = _options$encoder2 === undefined ? undefined : _options$encoder2;
        ...
        return new Promise((resolve, reject) => {
                ...
                case 'png':
                    {
                        if (!canJSEncodePng(this) || useCanvas) {
                            ...
                            buffer = (0, _fastPng.encode)(this, encoderOptions);
                        }
                        break;
                    }
                case 'jpg':
            ...
            if (stream) {
                var out = (0, _environment.createWriteStream)(path);
                out.on('finish', resolve);
                out.on('error', reject);
                stream.pipe(out);
            } else if (buffer) {
                (0, _environment.writeFile)(path, Buffer.from(buffer), err => {
                    if (err) {
                        reject(err);
                        return;
                    }
